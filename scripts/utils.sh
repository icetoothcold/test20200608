...
function get_master_nodename_strings
{
    python3 -c "import yaml; all=yaml.safe_load(open('$inventoryPath/$1/hosts.yml'))['all'];print(' '.join(host for host in all['hosts'].keys() if host in all['children']['kube-master']['hosts'] ))"
}
...
function get_infra_nodename_strings
{
